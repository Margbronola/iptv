...
import 'package:seizhiptv/viewmodel/cast_vm.dart';
...
class TVSeriesAPI {
  ...
  Future<void> topRatedTVShow() async {
    try {
      return dio
          .get(
            "${Network.tmdbDomain}/tv/top_rated?api_key=$_apiKey",
            options: Options(headers: {"accept": "application/json"}),
          )
          .then((response) {
            if (response.statusCode == 200) {
              final List data = response.data['results'];
              _topRatedTVShowViewModel.populate(
                data.map((e) => TopSeriesModel.fromJson(e)).toList(),
              );
              return;
            }
            return;
  ...
  Future<void> getTVVideos({required int id}) async {
    try {
      return dio
          .get(
            "${Network.tmdbDomain}/tv/$id/videos?api_key=$_apiKey",
            options: Options(headers: {"accept": "application/json"}),
          )
          .then((response) {
            if (response.statusCode == 200) {
              final List data = response.data['results'];
              _tvVideoViewModel.populate(
                data.map((e) => Video.fromJson(e)).toList(),
              );
              return;
            }
            return;
  ...
  Future<int?> searchTV({required String title, String? year}) async {
    try {
      return dio
          .get(
            "${Network.tmdbDomain}/search/tv?api_key=$_apiKey&query=$title&year=$year",
            options: Options(headers: {"accept": "application/json"}),
