...
import 'package:seizhiptv/models/cast.dart';
...
class MovieAPI {
      ...
      return dio
          ...
          .then((response) {
            if (response.statusCode == 200) {
              ...
              _topRatedMovieViewModel.populate(
                data.map((e) => TopMovieModel.fromJson(e)).toList(),
              );
              return;
            }
            return;
  ...
  Future<void> getMovieVideos({required int id}) async {
    try {
      return dio
          .get(
            "${Network.tmdbDomain}/movie/$id/videos?api_key=$_apiKey",
            options: Options(headers: {"accept": "application/json"}),
          )
          .then((response) {
            if (response.statusCode == 200) {
              final List data = response.data['results'];
              _movieVideoViewModel.populate(
